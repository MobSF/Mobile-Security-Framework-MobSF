...
def doScan(app, server_url):
    ...
    headers = {"Content-Type": content_type, 'content-length': str(len(body)), 'Cookie': 'csrftoken='+CSRF}
    request = tornado.httpclient.HTTPRequest(UPLOAD_URL, "POST", headers=headers, body=body, validate_cert=False)
    ...
    if response.code == 200:
        ...
        if r["status"] == "success":
            ...
            #Start Scan
            START_SCAN_URL = server_url + "/" + r["url"].replace(APP_NAME,urllib.quote(APP_NAME))
