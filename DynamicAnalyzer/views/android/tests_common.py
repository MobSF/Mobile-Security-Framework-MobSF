...
def exported_activity_tester(request):
    ...
    try:
        ...
        logger.info('Fetching exported activities for %s', package)
        exported_act = python_list(static_android_db[0].EXPORTED_ACT)
        if not exported_act:
            msg = 'No Exported activites found'
            logger.info(msg)
            data = {'status': 'failed',
                    'message': msg}
            return json_response(data)
    ...
    try:
        ...
        if not static_android_db.exists():
            data = {'status': 'failed',
                    'message': 'App details not found in database'}
            return json_response(data)
        logger.info('Fetching  activities for %s', package)
        activities = python_list(static_android_db[0].ACTIVITIES)
        if not activities:
            msg = 'No Activites found'
            logger.info(msg)
            data = {'status': 'failed',
                    'message': msg}
            return json_response(data)
