...
for pd in permission_divs:
	...
	online_permissions[permission_name] = [protection_level, "TODO - fill in short description", description]

# check the permissions we currently have in dvm_permissions.py
DVM_PERMISSIONS = {}
eval(compile(open("StaticAnalyzer/views/android/dvm_permissions.py").read(), "<string>", "exec"))
MANIFEST_PERMISSIONS = DVM_PERMISSIONS['MANIFEST_PERMISSION']

for permission_name in online_permissions.keys():
