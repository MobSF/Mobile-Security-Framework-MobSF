...
def _get_context_from_db_entry(db_entry):
...
def _get_context_from_db_entry_zip(db_entry):
    print "\n[INFO] Analysis is already Done. Fetching data from the DB..."
...
def _get_context_from_an_zip(app_dic, man_data_dic, man_an_dic, code_an_dic):
    context = {
        'title' : 'Static Analysis',
        'name' : app_dic['app_name'],
        'size' : app_dic['size'],
...
def _update_db_entry_zip(app_dic, man_data_dic, man_an_dic, code_an_dic):
    # pylint: disable=E1101
    StaticAnalyzerAndroid.objects.filter(MD5=app_dic['md5']).update(
        TITLE='Static Analysis',
        APP_NAME=app_dic['app_name'],
...
def _create_db_entry_zip(app_dic, man_data_dic, man_an_dic, code_an_dic):
    static_db = StaticAnalyzerAndroid(
        TITLE='Static Analysis',
        APP_NAME=app_dic['app_name'],
        SIZE=app_dic['size'],
        ...
        if (
            ...
            if typ == 'apk':
                ...
                if db_entry.exists() and rescan == '0':
                    context = _get_context_from_db_entry(db_entry)
                else:
                ...
                db_entry = StaticAnalyzerAndroid.objects.filter(MD5=app_dic['md5'])
                if db_entry.exists() and rescan == '0':
                    _get_context_from_db_entry_zip(db_entry)
                else:
