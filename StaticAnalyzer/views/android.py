        ...
        if (
            ...
            if typ == 'apk':
                ...
                if db.exists() and rescan == '0':
                    ...
                    app_dic['sha1'], app_dic['sha256'] = HashGen(app_dic['app_path'])  #SHA1 & SHA256 HASHES
                    ...
                    app_dic['persed_xml'] = GetManifest(app_dic['app_dir'],app_dic['tools_dir'],'',True)  #Manifest XML
                    ...
                    try:
                        ...
                        if rescan == '1':
                            ...
                            _update_db_entry(app_dic, man_data_dic, man_an_dic, code_an_dic, cert_dic)
                        elif rescan == '0':
                            print "\n[INFO] Saving to Database"
                            _create_db_entry(app_dic, man_data_dic, man_an_dic, code_an_dic, cert_dic)
                    except:
                        PrintException("[ERROR] Saving to Database Failed")
                    context = _get_context_from_an(app_dic, man_data_dic, man_an_dic, code_an_dic, cert_dic)
                ...
                if db.exists() and rescan == '0':
                    ...
                    if Valid and pro_type == 'ios':
                        ...
                        return HttpResponseRedirect('/StaticAnalyzer_iOS/?name='+app_dic['app_name']+'&type=ios&checksum='+app_dic['md5'])
                    app_dic['certz'] = GetHardcodedCertKeystore(app_dic['files'])
                    ...
                    if Valid and (pro_type in ['eclipse', 'studio']):
                        ...
                        app_dic['size'] = str(FileSize(app_dic['app_path'])) + 'MB'   #FILE SIZE
                        app_dic['sha1'],app_dic['sha256'] =  HashGen(app_dic['app_path'])        #SHA1 & SHA256 HASHES
                        app_dic['persed_xml'] =  GetManifest(app_dic['app_dir'],app_dic['tools_dir'],pro_type,False)   #Manifest XML
                        app_dic['mani'] = '../ManifestView/?md5='+app_dic['md5']+'&type='+pro_type+'&bin=0'
                        man_data_dic = ManifestData(app_dic['persed_xml'],app_dic['app_dir'])
                        ...
                        code_an_dic = CodeAnalysis(app_dic['app_dir'],app_dic['md5'],man_an_dic['permissons'],pro_type)
                        print "\n[INFO] Connecting to Database"
                        try:
                            #SAVE TO DB
                            if rescan == '1':
                                print "\n[INFO] Updating Database..."
                                _update_db_entry_zip(app_dic, man_data_dic, man_an_dic, code_an_dic, cert_dic)
                            elif rescan == '0':
                                print "\n[INFO] Saving to Database"
                                _create_db_entry_zip(app_dic, man_data_dic, man_an_dic, code_an_dic, cert_dic)
                        except:
                            PrintException("[ERROR] Saving to Database Failed")
                        context = _get_context_from_an_zip(app_dic, man_data_dic, man_an_dic, code_an_dic, cert_dic)
                    else:
        ...
        for dirName, subDir, files in os.walk(JS):
            for jfile in files:
                ...
                if jfile.endswith('.java') and (any(cls in repath for cls in settings.SKIP_CLASSES) == False):
                    ...
                    if (re.findall(r'MODE_WORLD_READABLE|Context\.MODE_WORLD_READABLE',dat) or re.findall(r'openFileOutput\(\s*".+"\s*,\s*1\s*\)',dat)):
                        c['d_con_world_readable'].append(jfile_path.replace(JS,''))
                    if (re.findall(r'MODE_WORLD_WRITABLE|Context\.MODE_WORLD_WRITABLE',dat) or re.findall(r'openFileOutput\(\s*".+"\s*,\s*2\s*\)',dat)):
                    ...
                    if (re.findall(r"System.loadLibrary\(|System.load\(", dat)):
                        native = True
                    if (re.findall(r'dalvik.system.DexClassLoader|java.security.ClassLoader|java.net.URLClassLoader|java.security.SecureClassLoader',dat)):
                        dynamic = True
                    ...
                    if (re.findall(r'sendBroadcast\(|sendOrderedBroadcast\(|sendStickyBroadcast\(',dat)):
